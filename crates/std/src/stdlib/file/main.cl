...
const open = fn(path: str, mode: str) -> ptr:<@u8> => c_fopen(path, mode);

const open_read = fn(path: str) -> ptr:<@u8> => c_fopen(path, "r");
const open_write = fn(path: str) -> ptr:<@u8> => c_fopen(path, "w");
const open_append = fn(path: str) -> ptr:<@u8> => c_fopen(path, "a");

const close = fn(handle: ptr:<@u8>) => {
    c_fclose(handle);
    null;
};

const flush = fn(handle: ptr:<@u8>) => {
    c_fflush(handle);
    null;
};

const write = fn(handle: ptr:<@u8>, content) => {
    let txt = "" & content;
...
const write_line = fn(handle: ptr:<@u8>, content) => {
...
const read_line = fn(handle: ptr:<@u8>, max: int) -> str => {
