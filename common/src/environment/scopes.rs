...
impl<T : RuntimeValue, U : RuntimeType> Environment<T, U> {
    pub fn find_checker_scope_at(&mut self, target : Location) -> ScopeSearchResult {
        ...
        let program = parser.produce_ast(fs::read_to_string(path).unwrap()).unwrap();

        self.find_checker_scope_at_recursive(root, root + 1, program, target)
    }

    fn find_checker_scope_at_recursive(&mut self, scope : u64, counter : u64, mut node : Node, target : Location) -> ScopeSearchResult {
        ...
        match node.node_type {
            NodeType::IterExpression { map: _, loop_type: _ , conditionals: _ } => {
                ScopeSearchResult::Failed { counter: counter + 1, scope }
            },
            NodeType::ScopeDeclaration { body, is_temp } => {
                ...
                for stmt in body.into_iter() {
                    match self.find_checker_scope_at_recursive(scope, counter, stmt, target.clone()) {
                        ...
                        ScopeSearchResult::Failed { counter: c, scope: _ } => counter = c,
                    };
                }
            ...
            NodeType::LoopDeclaration { loop_type: _, body } => {
                match self.find_checker_scope_at_recursive(scope + 1, counter + 1, *body, target.clone()) {
                    ScopeSearchResult::Success(x) => ScopeSearchResult::Success(x),
                    ScopeSearchResult::Failed { counter, scope: _ } => ScopeSearchResult::Failed { counter, scope },
                }
            }
            NodeType::ImplDeclaration { identifier: _, functions } => {
                let (scope, mut counter) = (scope + 1, counter + 1);

                for stmt in functions.into_iter() {
                    match self.find_checker_scope_at_recursive(scope, counter, stmt.0, target.clone()) {
                        ScopeSearchResult::Success(x) => return ScopeSearchResult::Success(x),
                        ScopeSearchResult::Failed { counter: c, scope: _ } => counter = c,
                    };
                }
            ...
            NodeType::ImportStatement { module, alias, values } => {
                if !values.is_empty() || alias.is_some() {
                    ...
                    if let Ok(x) = self.get_scope_list(scope, module).clone() {
                        let mut parser = calibre_parser::Parser::default();
                        let program = parser.produce_ast(fs::read_to_string(self.scopes.get(&x).unwrap().path.clone()).unwrap()).unwrap();
                        match self.find_checker_scope_at_recursive(scope + 1, counter + 1, program, target.clone()) {
                            ScopeSearchResult::Success(x) => ScopeSearchResult::Success(x),
                            ScopeSearchResult::Failed { counter, scope: _ } => ScopeSearchResult::Failed { counter, scope },
                        }
                    } else {
